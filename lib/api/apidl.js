...
function telesticker(url) {
    return new Promise(async (resolve, reject) => {
		try {
			...
			const response = await fetch(`https://api.telegram.org/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/getStickerSet?name=${encodeURIComponent(packName)}`,{method: "GET",headers: {"User-Agent": "GoogleBot"}})
			const data = await response.json();
			const hasil = await Promise.all(
				data.result.stickers.map(async (stiker) => {
					const fileId = stiker.thumb.file_id;
					const response2 = await fetch(`https://api.telegram.org/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/getFile?file_id=${fileId}`);
