              ...
              axios
                ...
                .then((resaudio) => {
                  ...
                  const result = {
                    ...
                    video2: ch(
                      "body > div.welcome.section > div > div:nth-child(3) > div.col.s12.l8 > a:nth-child(5)"
                    ).attr("href"),
                    video_HD: ch(
                    ...
                    audio: hc(
                      "body > div.welcome.section > div > div:nth-child(3) > div.col.s12.l8 > a:nth-child(6)"
                    ).attr("href"),
                    audio_Download: hc(
  ...
  return new Promise(async (resolve, reject) => {
    axios
      .get("https://id.pinterest.com/search/pins/?autologin=true&q=" + querry, {
        headers: {
          cookie:
            '_auth=1; _b="AXOtdcLOEbxD+qMFO7SaKFUCRcmtAznLCZY9V3z9tcTqWH7bPo637K4f9xlJCfn3rl4="; _pinterest_sess=; _ir=0',
        },
      })
      .then(({ data }) => {
        const $ = cheerio.load(data);
...
function igstory(username) {
  return new Promise(async (resolve, reject) => {
    axios
      .get("https://storydownloader.app/en", {
        headers: {
          "user-agent":
      ...
      .then((res) => {
        const $ = cheerio.load(res.data);
        var token = $("head > meta:nth-child(6)").attr("content");
        axios
          .request({
            url: "https://storydownloader.app/request",
            method: "post",
            data: new URLSearchParams(
              Object.entries({ username: username, _token: token })
            ),
            headers: {
              "user-agent":
                "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69 Safari/537.36",
          ...
          .then((respon) => {
            if (!respon.data.html) {
              ...
              $("div.row > div").each(function (a, b) {
                const urlDownload = $(b).find("a").attr("href");
                if (urlDownload !== "https://storydownloader.app/en") {
                  hasil.push({
              ...
              hasil = hasil.filter((item) => Object.keys(item).length !== 0);
    ...
    if (!data.data) {
      if (!data.items) {
        ...
        for (let i = 0; i < data.items.length; i++) {
          if (data.items[i].urls[0].extension === "mp4") {
            let result = {
              extension: data.items[i].urls[0].extension,
              video: data.items[i].urls[0].url,
            };
            hasil.push(result);
          } else {
            let result = {
              extension: data.items[i].urls[0].extension,
              image: data.items[i].pictureUrl,
              video: data.items[i].urls[0].url,
            };
            hasil.push(result);
          }
        ...
        let dataresult = {
          decs: data.items[0].meta.title,
          source: data.items[0].meta.sourceUrl,
          url: hasil,
        };
      ...
      try {
        resultdata = {
          url: data.items[0].urls,
          thumb: data.items[0].pictureUrl,
          decs: data.items[0].meta.title,
          source: data.items[0].meta.sourceUrl,
        };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const hasil = await Promise.all(
        data.result.stickers.map(async (stiker) => {
          const fileId = stiker.thumb.file_id;
