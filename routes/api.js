...
const mahiru = require("../lib/listdl");
  ...
  async (req, res, next) => {
    ...
    if (!username)
      return res.json({
        status: false,
        creator: `${creator}`,
        message: "[!] insert username parameter",
      });

    mahiru.igstory(username).then(async (data) => {
...
router.get("/api/textpro/glitch2", cekKey, async (req, res, next) => {
  ...
  if (!text2)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert text parameter2",
    });
  mahiru
    .textpro(
...
router.get("/api/soundoftext", cekKey, async (req, res, next) => {
  ...
  if (!lan)
    return res.json({
      ...
      creator: `${creator}`,
      message:
        "[!] please put the lang format correctly check the website https://soundoftext.com/docs to see the lang code",
    });
...
router.get("/api/randomgambar/couplepp", cekKey, async (req, res, next) => {
  let resultt = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/kopel.json"
  );
...
router.get("/api/randomgambar/dadu", cekKey, async (req, res, next) => {
  let dadu = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/dadu.json"
  );
...
router.get("/api/game/susunkata", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/susunkata.json"
  );
...
router.get("/api/game/tembakbendera", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/tebakbendera.json"
  );
  let ha = ra[Math.floor(Math.random() * ra.length)];
  limitapikey(req.query.apikey);
  res.json({
...
router.get("/api/game/tembakgame", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/tebakgame.json"
  );
  let ha = ra[Math.floor(Math.random() * ra.length)];
  limitapikey(req.query.apikey);
  res.json({
...
router.get("/api/game/tembakkata", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/tebakkata.json"
  );
  let ha = ra[Math.floor(Math.random() * ra.length)];
  limitapikey(req.query.apikey);
  res.json({
...
router.get("/api/game/tembaklirik", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/tebaklirik.json"
  );
  let ha = ra[Math.floor(Math.random() * ra.length)];
  limitapikey(req.query.apikey);
  res.json({
...
router.get("/api/game/tembaklagu", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/tebaklagu.json"
  );
  let ha = ra[Math.floor(Math.random() * ra.length)];
  limitapikey(req.query.apikey);
  res.json({
...
router.get("/api/game/tembakkimia", cekKey, async (req, res, next) => {
  let ra = await fetchJson(
    "https://raw.githubusercontent.com/mahiruBot/data-rest-api/main/tebakkimia.json"
  );
  let ha = ra[Math.floor(Math.random() * ra.length)];
  limitapikey(req.query.apikey);
  res.json({
...
router.get("/api/maker/darkness", cekKey, async (req, res) => {
  ...
  if (!no)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert number parameter",
    });
...
router.get("/api/maker/emojimix", cekKey, async (req, res, next) => {
  ...
  if (!emoji1)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert emoji1 parameter",
    });
  if (!emoji2)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert emoji2 parameter",
    });
...
router.get("/api/maker/welcome1", cekKey, async (req, res, next) => {
  ...
  if (!name)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert name parameter",
    });
  if (!grup)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert gpname parameter",
    });
  if (!member)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert member parameter",
    });
  if (!pp)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert pp parameter",
    });
  if (!bg)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert bg parameter",
    });
  ...
  if (!imgpp)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] check image url back pp",
    });
  if (!bgimg)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] check image url back bg",
    });
...
router.get("/api/maker/goodbye1", cekKey, async (req, res, next) => {
  ...
  if (!imgpp)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert pp link parameter",
    });
  if (!bgimg)
    return res.json({
      status: false,
      creator: "mahiru",
      message: "[!] insert bg link parameter",
    });
...
router.get("/api/linkshort/tinyurl", cekKey, async (req, res, next) => {
  ...
  if (!islink)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert url parameter only",
    });
  ...
  async (req, res, next) => {
    ...
    if (!alias)
      return res.json({
        status: false,
        creator: `${creator}`,
        message: "[!] insert alias parameter",
      });
...
router.get("/api/info/translate", cekKey, async (req, res, next) => {
  ...
  if (!lang)
    return res.json({
      ...
      creator: `${creator}`,
      message:
        "[!] enter parameters. You can see the list of languages ​​at https://cloud.google.com/translate/docs/languages",
    });
  ...
  translate(text, { to: lang })
    ...
    .catch((err) => {
      res.json({
        ...
        creator: `${creator}`,
        message:
          "[!] enter the lang parameter Correctly. You can see the list of languages ​​at https://cloud.google.com/translate/docs/languages",
      });
    });
});
...
router.get("/api/info/emoji", cekKey, async (req, res, next) => {
  ...
  if (!emoji1)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert emoji parameter",
    });
  ...
  if (hasil == null)
    return res.json({
      status: false,
      creator: `${creator}`,
      message: "[!] insert only one emoji parameter",
    });
  limitapikey(req.query.apikey);
  res.json({
    status: true,
